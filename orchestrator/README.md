...
1. `DataEngineeringAgent(output_path=self.analytical_path).run(raw_dataset)` → `CleanedDataset`
...
`analytical_path` / `dashboard_layout_path` are constructor params (default
to each agent's own default path) so tests can point them at a temp dir.
...
installed too — see `requirements.txt`. This works for the current
single-process design; a distributed setup (agents as separate services
called over RPC) would need a different dependency/build story.
